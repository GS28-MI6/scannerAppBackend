...
module.exports = server => {
  ...
  server.post("/item", function(req, res) {
    ...
    connection.query("SELECT * FROM productos WHERE barcode=? AND cliente=?",[barcode, id_cliente], function(error, results, fields) {
      ...
      if (error){
        res.status(200).send({ ErrorCode: 400, Errors: ["El producto no existe en la base de datos"], Response: {error} });
      }else{
  ...
  server.post("/items", function(req, res) {
    ...
    connection.query("SELECT * FROM productos WHERE cliente=? ORDER BY stock DESC",[req.body.id_cliente], function(error, results, fields) {
      ...
      if (error){
        res.status(200).send({ ErrorCode: 400, Errors: ["Fallo al obtener los productos"], Response: {error} });
      }else{
        let jsonResponse= JSON.stringify(results)
        res.status(200).send({ ErrorCode: 0, Errors: [""], Response: {jsonResponse} });
      }
            ...
            connection.query(
              ...
              function(error, results, fields) {
                if (!error) {
                  ...
                  res.status(200).send({ ErrorCode: 400, Errors: ["Fallo al reducir el stock"], Response: {error} });
                }
              }
            );
          } else {
            res.status(200).send({ ErrorCode: 400, Errors: ["Fallo al insertar nueva venta"], Response: {error} });
          }
        }
      );
    })
    connection.query(
      ...
      function(error, results, fields) {
        if (!error) {
          ...
          res.status(200).send({ ErrorCode: 400, Errors: ["Fallo al realizar la venta"], Response: {error} });
        }
      }
    );
  });
  ...
  server.post("/ingreso", function(req, res, next) {
    try {
              ...
              connection.query(
                ...
                function(error, results, fields) {
                  if (!error) {
                    ...
                    res.status(200).send({ ErrorCode: 400, Errors: ["Fallo al agregar un producto"], Response: {error} });
                  }
                }
              );
            } else {
              connection.query(
                ...
                function(error, results, fields) {
                  if (!error) {
                    ...
                    res.status(200).send({ ErrorCode: 400, Errors: ["Fallo al actualizar el producto"], Response: {error} });
                  }
                }
              );
            }
      ...
      res.status(200).send({ ErrorCode: 401, Errors: ["Fallo al actualizar el producto"], Response: {error} });
    }
  });
  ...
  server.post("/countFiltered", function(req, res) {
    ...
    connection.query(
      ...
      function(error, results, fields) {
        if (error){
          res.status(200).send({ ErrorCode: 400, Errors: ["Fallo al calcular las ventas"], Response: {error} });
        }else{
          let jsonResponse= JSON.stringify(results)
          res.status(200).send({ ErrorCode: 0, Errors: [""], Response: {jsonResponse} });
        }
